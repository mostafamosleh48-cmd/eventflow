...
### Scaffold & Tooling
- [ ] 1.1 `npm init` and configure `package.json` (name, scripts, engines)
- [ ] 1.2 Install dev dependencies: `typescript`, `eslint`, `prettier`, `jest`, `ts-jest`, `@types/node`
- [ ] 1.3 Install runtime dependencies: `express`, `pg`, `pg-boss`, `dotenv`
- [ ] 1.4 Install type packages: `@types/express`, `@types/pg`
- [ ] 1.5 Create `tsconfig.json` (strict: true, outDir: dist, rootDir: src)
- [ ] 1.6 Create `.eslintrc` config (TypeScript parser, no-any rule)
- [ ] 1.7 Create `.prettierrc` (2-space indent, single quotes, trailing commas, semicolons)
- [ ] 1.8 Add npm scripts: `dev`, `build`, `start`, `worker`, `lint`, `format`, `test`, `migrate`
- [ ] 1.9 Create `src/` directory structure: `server.ts`, `worker.ts`, `routes/`, `actions/`, `services/`, `db/`, `types/`, `utils/`
- [ ] 1.10 Create `tests/` directory structure: `tests/unit/`, `tests/integration/`
- [ ] 1.11 Create `.gitignore` (node_modules, dist, .env, coverage)

### Docker
- [ ] 1.12 Create `Dockerfile` (multi-stage: build + production, Node 20 base)
- [ ] 1.13 Create `docker-compose.yml` (api, worker, postgres services)
- [ ] 1.14 Create `.dockerignore`
- [ ] 1.15 Verify `docker compose up --build` starts all services cleanly

### Database
- [ ] 1.16 Create `.env.example` with all required/optional env vars
- [ ] 1.17 Create `src/db/pool.ts` — PostgreSQL connection pool using `pg.Pool`
- [ ] 1.18 Create migration runner script (`src/db/migrate.ts`)
- [ ] 1.19 Migration 001: Create `pipelines` table
- [ ] 1.20 Migration 002: Create `subscribers` table
- [ ] 1.21 Migration 003: Create `jobs` table
- [ ] 1.22 Migration 004: Create `delivery_attempts` table
- [ ] 1.23 Verify migrations run successfully against Dockerized PostgreSQL
